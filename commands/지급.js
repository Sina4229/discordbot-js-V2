...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!betting) return message.reply("**사용법 : !지급 @멘션 (금액)**") // 2번쨰 args에서 숫자를 못찾으면 리턴
        if (message.content.includes("-")) return message.reply("**금액엔 -가 포함되면 안돼요**") // 메세지 내용에 -가 포함되면 리턴
        if (betting < 1000) return message.reply("**최소 송금 금액은 1000원입니다**") // 2번쨰 args가 1000보다 작으면 리턴
        const money = parseInt(sk.money) // money를 메세지를 보낸사람의 돈으로 지정
        const money2 = parseInt(tkdeoqkd.money) // money2를 멘션한 유저의 돈으로 지정
        if (money < betting) return message.reply("**가진 돈보다 많은 금액을 지급할 수 없습니다**")
        if (message.author.id == user.id) return message.reply("**자신한텐 지급할 수 없습니다.**") // 맨션 한 유저가 메세지를 보낸유저와 아이디가 같다면 return
