...
module.exports = {
    ...
    async execute(message) {
        if (cooldown.has(message.author.id)) {
            return message.channel.send("**오류 ) 미션 명령어는 40초이라는 쿨타임이 존재합니다. 잠시 후 다시 시도해 주세요**");
        } else {
        ...
        if (!emdfhr) return message.reply("**오류 ) 등록되지 않은 유저입니다. ( !돈줘 )**")
        ...
        try {
            ...
            if (wait) {
                message.channel.send(`**문제를 맞추셨어요 🤩 20000원을 드릴게요 \n현재 잔액 : ${emdfhr.money + 20000}원**`)
                await Schema.findOneAndUpdate({ userid: message.author.id }, {
                    money: emdfhr.money + 20000,
